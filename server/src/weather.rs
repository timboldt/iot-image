...
pub fn generate_weather_svg(weather: &WeatherData) -> String {
    let mut svg = String::from(r#"<svg xmlns="http://www.w3.org/2000/svg" width="800" height="480" viewBox="0 0 800 480">"#);
    svg.push_str("\n");
    ...
    svg.push_str(&format!(r#"  <text x="320" y="{}" font-family="Arial" font-size="20" fill="black">{}% ({})</text>"#,
        y_current, weather.current.humidity, humidity_text(weather.current.humidity, weather.current.temp)));
    svg.push_str("\n");

    if let Some(w) = weather.current.weather.first() {
        svg.push_str(&format!(r#"  <text x="450" y="{}" font-family="Arial" font-size="20" fill="black">{}</text>"#,
            y_current, w.main));
        svg.push_str("\n");
    }

    // 5-day forecast
    svg.push_str(&format!(r#"  <text x="20" y="130" font-family="Arial" font-size="24" font-weight="bold" fill="black">5-Day Forecast:</text>"#));
    svg.push_str("\n");
    ...
    for (day_idx, day) in weather.daily.iter().take(days_to_show).enumerate() {
        ...
        let day_name = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"][day_time.weekday() as usize];
