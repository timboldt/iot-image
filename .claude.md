...
- **Display:** 7.3-inch full-color e-paper display (GDEP073E01), 800x480 pixels, 8 colors (black, white, red, green, blue, yellow, orange)
...
- **PNGdec** - PNG decoder library (memory-based decoding)

### Important Implementation Details

#### PNG Decoding Strategy
- **Memory-based decoding**: The PNGdec library requires the entire PNG to be in memory for seeking during decode
- Download complete PNG into dynamically allocated buffer first
- Then decode from memory buffer using custom callbacks (`png_open`, `png_close`, `png_read`, `png_seek`)
- Position tracking via `png_buffer_pos` global variable
- Free memory after decoding completes

#### Color Mapping for E-ink
The `png_draw()` callback converts RGB565 to 8-color e-ink palette:
- White (0xFFFF) → GxEPD_WHITE
- Black (0x0000) → GxEPD_BLACK
- Reddish → GxEPD_RED
- Greenish → GxEPD_GREEN
- Blueish → GxEPD_BLUE
- Yellowish → GxEPD_YELLOW
...
```
1. Connect to WiFi
2. Download entire PNG into memory buffer (malloc/free)
3. Decode PNG using PNGdec with custom callbacks
4. Render pixels via png_draw callback (draws directly to display buffer)
5. Refresh e-ink display (takes ~20 seconds)
6. Disconnect WiFi
7. Enter deep sleep for 6 hours
8. Wake and repeat
```
...
- **Server exposes HTTP endpoint** to serve generated image file
  - Endpoint: `/image/latest` (configured in `config.h`)
  - Returns binary PNG data

- **Client queries endpoint periodically**
  - Request: `GET http://pidev.local:8080/image/latest`
  - Downloads into memory buffer
  - Timeout: 30 seconds
...
- Expected output: WiFi connection status, HTTP download progress, PNG decode status, display refresh timing
...
4. Monitor serial output for any WiFi/HTTP/PNG errors

## Common Issues & Solutions

### PNG Decode Errors
- Error code 1: Usually means streaming was attempted instead of memory-based decoding
- Solution: Ensure entire PNG is downloaded into `png_buffer` before calling `png.open()`
- The library needs to seek within the data, which doesn't work with HTTP streams

### Memory Allocation
- ESP32-S3 has limited RAM (~300KB free for sketches)
- PNG files should be kept under 200KB
- Always free PNG buffer after decoding: `free(png_buffer)`
